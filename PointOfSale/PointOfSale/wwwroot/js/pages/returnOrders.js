    ...
    function () {
        ...
        return {
            init: function () {
                ...
                $('#kt_ecommerce_return_table').off().on("click", "button.details-control", function () {
                    ...
                    if (row.child.isShown()) {
                        ...
                        $('#in' + row.data().id).DataTable().destroy();
                    }
                    else {
                        ...
                        var childTable = $('#in' + id).DataTable({
                            "bServerSide": false,
                            ajax: {
                                "url": '/Order/GetReturnOrderDetail',
                                "type": "POST",
                                "data": {
                                    id: id,
                                }
                            ...
                            columns: [
                                ...
                                { data: "salePrice", "sortable": false, "className": "text-end", render: $.fn.dataTable.render.number(',', '.', 2) },
                                { data: "quantity", "sortable": false, "className": "text-end", render: $.fn.dataTable.render.number(',', '.', 0) },
                                { data: "subTotal", "sortable": false, "className": "text-end", render: $.fn.dataTable.render.number(',', '.', 2) },
                            ],
                            "destroy": true,
                            select: false,
                            "bProcessing": false,
...
function formatTable(rowData) {
    var childTable = '<table id="in' + rowData.id + '" class="table table-striped table-bordered responsive no-wrap" width="100%">' +
        '<thead><th>Product Name</th><th>Sale Price</th><th>Qty</th><th>Total</th>' +
        '</thead> ' +
        '</table>';
...
function getId(ele) {
    ...
    $.post('/order/GetReturnOrderInvoiceDetails', { id: id }, function (response) {
