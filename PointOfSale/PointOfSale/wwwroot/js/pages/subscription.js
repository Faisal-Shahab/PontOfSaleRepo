...
function initTable() {
    $('#subscriptionDataTable').DataTable({
        ...
        "columns": [
            ...
            { data: "maxAccounts", "sortable": false },
            { data: "maxOrders", "sortable": false },
            {
                "data": null,
...
$('#add-new').on('click', function () {
    ...
    $(".modal-body").load("/Subscription/Create", function () {
        initEvents();
    });
...
function getId(event, buttonName) {
    ...
    if (buttonName == 'edit') {
        $('#kt_modal_4_2').modal('show');
        $(".modal-body").load('/Subscription/Edit?id=' + id, function () {
            initEvents();
        });
        $('.modal-title').text("Subscription Details");
