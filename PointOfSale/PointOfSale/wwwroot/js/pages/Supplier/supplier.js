"use strict";
var KTAppEcommercesuppliers =
    function () {
        var t, e, n = () => {
            t.querySelectorAll('[data-kt-ecommerce-supplier-filter="delete_row"]').
                forEach((t => {
                    t.addEventListener("click", (function (t) {
                        ...
                        Swal.fire({
                            ...
                            if (t.value) {
                                $.post('/supplier/Delete', { id: o }, function (res) {
                                    if (res.status) {
                                        Swal.fire({
        ...
        return {
            init: function () {
                (t = document.querySelector("#kt_ecommerce_supplier_table")) && ((e = $(t).DataTable({
                    //info: !1,
                    //order: [],
                    "bServerSide": true,
                    "ajax": {
...
<button class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" onclick="KTAppEcommercesuppliers.getId(this,'payment')">
...
<button class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" onclick="KTAppEcommercesuppliers.getId(this,'edit')">
                                                                            <span class="svg-icon svg-icon-3">
                        ...
                        <button class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" data-kt-ecommerce-supplier-filter="delete_row">
                        <span class="svg-icon svg-icon-3">
																				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    ...
                    n()
                })), document.querySelector('[data-kt-ecommerce-supplier-filter="search"]').addEventListener("keyup", (function (t) {
                    e.search(t.target.value).draw()
                })), n())

                $('#kt_ecommerce_supplier_table').off().on("click", "button.details-control", function () {
                    ...
                    if (row.child.isShown()) {
                        ...
                        $('#in' + row.data().supplierId).DataTable().destroy();
                    }
                    else {
                        ...
                        var childTable = $('#in' + id).DataTable({
                            "bServerSide": false,
                            ajax: {
                                "url": '/Supplier/GetSupTransactions',
                                "type": "POST",
                                "data": {
                                    id: id,
                                }
            ...
            getId: function (ele, button) {
                ...
                if (button === 'edit')
                    ...
                    window.location.href = '/supplier/supplierPayment/' + id;
            }
        }
    }();

function formatTable(rowData) {
    var childTable = '<table id="in' + rowData.supplierId + '" class="table table-striped table-bordered responsive no-wrap" width="100%">' +
